...
public class ExpressionSplitter {
	...
	private ASTterm split(ASTadditiveArithmeticTerm additiveTerm, int fIndex, ArrayList<ASTatom> newAtoms) {
		...
		if (fIndex == 0) {
			...
			// create new variable
			ASTvar newVar = new ASTvar(ElpsTranslatorTreeConstants.JJTVAR);
			...
			ASTterm newTerm=new ASTterm(ElpsTranslatorTreeConstants.JJTTERM);
			ASTarithmeticTerm newATerm=new ASTarithmeticTerm(ElpsTranslatorTreeConstants.JJTARITHMETICTERM);
			ASTadditiveArithmeticTerm newAdTerm=new ASTadditiveArithmeticTerm(ElpsTranslatorTreeConstants.JJTADDITIVEARITHMETICTERM);
	...
	private ASTterm split(ASTmultiplicativeArithmeticTerm mTerm, int fIndex,
		...
		if (fIndex == 0) {
			...
			ASTarithmeticTerm newATerm=new ASTarithmeticTerm(ElpsTranslatorTreeConstants.JJTARITHMETICTERM);
			ASTadditiveArithmeticTerm newAdTerm=new ASTadditiveArithmeticTerm(ElpsTranslatorTreeConstants.JJTADDITIVEARITHMETICTERM);
			ASTmultiplicativeArithmeticTerm newMTerm=new ASTmultiplicativeArithmeticTerm(ElpsTranslatorTreeConstants.JJTMULTIPLICATIVEARITHMETICTERM);
	...
	private ASTterm split(ASTatomicArithmeticTerm atomTerm, 
          ...
          if(atomTerm.jjtGetNumChildren()>0) {
        	  ...
        	  if(child.getId()==ElpsTranslatorTreeConstants.JJTVAR) {
	...
	private ASTatom createEqAtom(ASTterm left, ASTterm right) {
		ASTatom newAtom = new ASTatom(ElpsTranslatorTreeConstants.JJTATOM);
