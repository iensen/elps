...
import parser.ElpsTranslatorConstants;
...
public class TermFetcher {
	...
	public HashMap<ASTterm, String> fetchTermSorts(ASTunlabeledProgramRule rule) {
		...
		String weakSep = ElpsTranslatorConstants.tokenImage[ElpsTranslatorConstants.WEAKSEP];
		// remove quotes:
	...
	private HashMap<ASTterm, String> fetchTermSorts(ASTatom atom) {
		...
		if (((SimpleNode) atom.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTEXTENDEDNONRELATOM) {
			unionMaps(result,
	 ...
	 *         obey sort definitions
	 */
	HashMap<ASTterm, String> fetchTermSorts(
			...
			if (((SimpleNode) (choice_element.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTNONRELATOM) {
				unionMaps(result,
						fetchTermSorts((ASTnonRelAtom) choice_element
								.jjtGetChild(i)));
			} else if (((SimpleNode) (choice_element.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTEXTENDEDSIMPLEATOMLIST) {
				unionMaps(
						result,
