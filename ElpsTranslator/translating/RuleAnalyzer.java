...
public class RuleAnalyzer {
	...
	public boolean isCrRule() {
		if (((SimpleNode) (rule.jjtGetChild(0))).getId() == ElpsTranslatorTreeConstants.JJTUNLABELEDPROGRAMCRRULE) {
			return true;
		} else {
			ASTunlabeledProgramRule urule = (ASTunlabeledProgramRule) rule
					.jjtGetChild(0);
			String crsep = ElpsTranslatorConstants.tokenImage[ElpsTranslatorConstants.CRRSEP];
			// remove quotes:
