...
public class TypeChecker {
	...
	private void checkProgramRule(ASTprogramRule rule) throws ParseException {
		if (((SimpleNode) rule.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTUNLABELEDPROGRAMRULE) {
			checkUnlabeledProgramRule((ASTunlabeledProgramRule) rule
					.jjtGetChild(0));
		} else if (((SimpleNode) rule.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTUNLABELEDPROGRAMCRRULE) {
	...
	private void checkUnlabeledProgramRule(ASTunlabeledProgramRule rule)
		...
		if (rule.image.trim().equals(":~")) {// weak constraint
			...
			checkBody(((ASTbody) (rule.jjtGetChild(0))));
				...
				&& ((SimpleNode) rule.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTPREDSYMBOL) {
			/*
		...
		} else if (((SimpleNode) rule.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTHEAD) {
			checkHead((ASThead) (rule.jjtGetChild(0)));
		} else {
			checkBody((ASTbody) (rule.jjtGetChild(0)));
		}
	...
	public void checkAtom(ASTatom atom) throws ParseException {
		if (((SimpleNode) atom.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTAGGREGATE) {
			checkAggregate((ASTaggregate) atom.jjtGetChild(0));
		} else if (((SimpleNode) atom.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTEXTENDEDNONRELATOM) {
	...
	private void checkExtendedNonRelAtom(ASTextendedNonRelAtom atom) //HERE
		...
		if (atom.jjtGetNumChildren() == 1) {// 0- arity predicate
			list = new ASTtermList(ElpsTranslatorTreeConstants.JJTTERMLIST);
		} else {
	...
	private void checkTermListWithEx(ASTtermList termList,
		...
		for (int i = 0; i < termList.jjtGetNumChildren(); i++) {
			...
			if(child.getId()==ElpsTranslatorTreeConstants.JJTARITHMETICTERM) {
	...
	private boolean checkNonGroundTerm(ASTterm termToCheck) {
		SimpleNode child = (SimpleNode) termToCheck.jjtGetChild(0);
		switch(child.getId()) {
	    	case ElpsTranslatorTreeConstants.JJTARITHMETICTERM:
	    		return checkConstants(termToCheck);
	    	case ElpsTranslatorTreeConstants.JJTSYMBOLICTERM:
	...
	private boolean checkNonGroundTerm(ASTsymbolicTerm symTerm) {
		...
		if (child0.getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICCONSTANT) {
	...
	private boolean checkConstants(SimpleNode n) {
		if (n.getId() == ElpsTranslatorTreeConstants.JJTATOMICARITHMETICTERM) {
	...
	private void checkAggregate(ASTaggregate agr) throws ParseException {
		for (int i = 0; i < agr.jjtGetNumChildren(); i++) {
			if (((SimpleNode) (agr.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTAGGREGATEELEMENTS) {
	...
	private void checkAggregateElement(ASTaggregateElement argelem)
		...
		for (int i = 0; i < argelem.jjtGetNumChildren(); i++) {
			if (((SimpleNode) (argelem.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTNONRELATOM) {
				checkNonRelAtom((ASTnonRelAtom) argelem.jjtGetChild(i));
			} else if (((SimpleNode) (argelem.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTEXTENDEDSIMPLEATOMLIST) {
	...
	private void checkSimpleAtom(ASTsimpleAtom simpleAtom)
		...
		if (simpleAtom.jjtGetNumChildren() > 0
				&& ((SimpleNode) (simpleAtom.jjtGetChild(0))).getId() == ElpsTranslatorTreeConstants.JJTEXTENDEDNONRELATOM) {
	...
	private void checkHead(ASThead head) throws ParseException {
		if (((SimpleNode) (head.jjtGetChild(0))).getId() == ElpsTranslatorTreeConstants.JJTDISJUNCTION) {
	...
	private void checkChoiceRule(ASTchoice_rule choice_rule)
		...
		for (int i = 0; i < choice_rule.jjtGetNumChildren(); i++) {
			if (((SimpleNode) (choice_rule.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTCHOICE_ELEMENTS) {
	...
	private void checkUnlabeledProgramCrRule(ASTunlabeledProgramCrRule crrule)
		...
		for (int i = 0; i < crrule.jjtGetNumChildren(); i++) {
			if (((SimpleNode) (crrule.jjtGetChild(i))).getId() == ElpsTranslatorTreeConstants.JJTHEAD) {
	...
	private boolean checkTerm(ASTterm symterm, ASTsortExpression expr) {
		if (((SimpleNode) (symterm.jjtGetChild(0))).getId() == ElpsTranslatorTreeConstants.JJTVAR) {
			return true;
		}
		//
		...
		case ElpsTranslatorTreeConstants.JJTFUNCTIONALSYMBOL:
	...
	private boolean checkTerm(ASTterm term, ASTbasicSort basicSort) {
		...
		case ElpsTranslatorTreeConstants.JJTSORTNAME:
			return checkTerm(term,
					sortNameToExpression.get(((ASTsortExpression) basicSort
							.jjtGetChild(0)).image));
		case ElpsTranslatorTreeConstants.JJTCONSTANTTERMLIST:
	...
	boolean checkTerm(ASTterm term, ASTunarySetExpression expr) {
		...
		case ElpsTranslatorTreeConstants.JJTCURLYBRACKETS:
	...
	boolean checkTerm(ASTterm term, String termName,
		...
		if (((SimpleNode) term.jjtGetChild(0)).getId() != ElpsTranslatorTreeConstants.JJTSYMBOLICTERM) {
			return false;
		} else {
