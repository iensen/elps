...
public class Unifier {
	...
	private boolean unify(ASTterm termWithVariables, ASTterm groundTerm) {
		...
		if (((SimpleNode) termWithVariables.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTVAR) {
			return true;
		} else if (((SimpleNode) termWithVariables.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICTERM) {
			if (((SimpleNode) groundTerm.jjtGetChild(0)).getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICTERM) {
				return unify(
	...
	private boolean unify(ASTsymbolicTerm termWithVariables,
		...
		if (child1.getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICFUNCTION) {
			return false;
		} else {
	...
	private boolean unify(ASTsymbolicTerm termWithVariables,
		...
		if (child1.getId() != child2.getId()) {
			return false;
		} else if (child1.getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICCONSTANT) {
	...
	private ASTterm createTerm(String content) {
		...
		p.token_source.SwitchTo(ElpsTranslatorConstants.IN_PROGRAM_RULES);
		ASTterm result = null;
		try {
