...
public class ASTterm extends SimpleNode {
	...
	public ASTterm(ElpsTranslator p, int id) {
		super(p, id);
	}

	public ASTterm(long value) {
		super(ElpsTranslatorTreeConstants.JJTTERM);
	...
	public ASTterm(String image) {
		...
		if (recordContents != null) {
            ...
            if(isNumericConstant(image)) {
				...
				ASTvar var = new ASTvar(ElpsTranslatorTreeConstants.JJTVAR);
				var.image = image;
	...
	public static ASTterm createArithmeticVarTerm(ASTvar var) {
		ASTterm term=new ASTterm(ElpsTranslatorTreeConstants.JJTTERM);
		ASTarithmeticTerm arTerm=new ASTarithmeticTerm(ElpsTranslatorTreeConstants.JJTARITHMETICTERM);
		ASTadditiveArithmeticTerm adTerm=new ASTadditiveArithmeticTerm(ElpsTranslatorTreeConstants.JJTADDITIVEARITHMETICTERM);
		adTerm.image="+";
		ASTmultiplicativeArithmeticTerm multTerm=new ASTmultiplicativeArithmeticTerm(ElpsTranslatorTreeConstants.JJTMULTIPLICATIVEARITHMETICTERM);
		ASTatomicArithmeticTerm atTerm=new ASTatomicArithmeticTerm(ElpsTranslatorTreeConstants.JJTATOMICARITHMETICTERM);
		atTerm.image="";
	...
	public boolean isVariable() {
		...
		return child.getId() == ElpsTranslatorTreeConstants.JJTVAR;
	}

	public boolean isRecord() {
		SimpleNode child = (SimpleNode) this.jjtGetChild(0);
		if (child.getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICTERM) {
			SimpleNode childOfChild = (SimpleNode) child.jjtGetChild(0);
			return childOfChild.getId() == ElpsTranslatorTreeConstants.JJTSYMBOLICFUNCTION;
		} else {
			return false;
		}
	}
